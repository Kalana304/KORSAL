...
for vid=1:NumVideos
    ...
    numfs(vid) = length(sortdirlist([videoDetDirBase,'*.mat']));
...
save_dir = [detresultpathBase,'/results/'];
if ~isdir(save_dir)
    mkdir(save_dir)
end

save([save_dir,'ucf101timing.mat'],'numfs','timimngs')
...
for f = 1 : numframes
    ...
    loc(loc(:,4)>239,4) = 239;
    loc = loc + 1;
    frames(f).baseBoxes = loc(1:min(size(loc,1),100),:);
    scores_arr = [scores(:,2:end),scores(:,1)];
    frames(f).baseScores = scores_arr(1:min(size(loc,1),100),:);
    ...
    loc(loc(:,4)>239,4) = 239;
    loc = loc + 1;
    frames(f).topBoxes = loc(1:min(size(loc,1),100),:);;
    scores_arr = [scores(:,2:end),scores(:,1)];
    frames(f).topScores = scores_arr(1:min(size(loc,1),100),:);
