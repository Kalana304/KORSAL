        ...
        if length(paths(lp).foundAT)>gap
            ...
            while i <= length(paths(lp).scores)
                ...
                if count == 1 || diff_found == 1
                    ...
                    for d = 1 : diff_found-1
                        gap_filled_paths(g_count).boxes(count,:) = paths(lp).boxes(i-1,:);
                        gap_filled_paths(g_count).scores(count) = paths(lp).scores(i-1);
                        gap_filled_paths(g_count).allScores(count,:) = paths(lp).allScores(i-1,:);
                        count = count + 1;
                    end
                    gap_filled_paths(g_count).boxes(count,:) = paths(lp).boxes(i,:);
                    gap_filled_paths(g_count).scores(count) = paths(lp).scores(i);
