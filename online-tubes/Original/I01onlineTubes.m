...
for index = 1:count-1
    ...
    if exist(opts.detDir,'dir')
        ...
        actionPaths(opts);
        %% Perform temporal labelling and evaluate; results saved in results cell
...
%% save results
save_dir = sprintf('%s/%s/%s_%s/results/',save_root, dataset, imgType, exp_id);
if ~isdir(save_dir)
    mkdir(save_dir)
end
% save_dir
