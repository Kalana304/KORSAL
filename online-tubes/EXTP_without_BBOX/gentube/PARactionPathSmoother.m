...
alltubes  = cell(length(actionpaths),1);        % a cell of num_actionPaths x 1
...
parfor t = 1 : length(actionpaths)
    ...
    fprintf('[doing for %s %d]\n',video_id,t);
    alltubes{t} = actionPathSmoother4oneVideo(actionpaths(t).paths,alpha,num_action,video_id) ; % row for each action (function returns the same structure as final_tubes but for a video) 
end
...
for t = 1 : length(actionpaths)
    ...
    for  k=1:length(vid_tubes.ts)       % for each tube append the start value, ts, te and others into the struture
        fprintf('%d/%d: %d/%d, action counts = %d\r', t, length(actionpaths), k, length(vid_tubes.ts), action_count)
...
%     alpha = alpha-3.2; 
    % Looping over the actions (24)
    for a = 1 : num_action
        action_paths = video_paths{a};
        num_act_paths = getPathCount(action_paths);
        for p = 1 : num_act_paths
            M = action_paths(p).allScores(:,1:num_action)'; %(:,1:num_action)'; % num_actions x n_boxes after transpose 
            %M = normM(M);
            %M = [M(a,:),1-M(a,:)];
            M = M + 20;                     % shift every value by 20 
