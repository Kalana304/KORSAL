...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--task', default='doubleSM',     # change to ctdet_smd
                             help='ctdet | ctdet_smd | ddd | multi_pose | exdet | double | track | sm | doubleSM')
    self.parser.add_argument('--dataset', default='hmdb21',   # change to ucf24
                             help='coco | kitti | coco_hp | pascal | ucf24 | hmdb21 | smd | seaships | flir')
    self.parser.add_argument('--input_type', default='rgb',
                             help='rgb | brox | fastOF')
    self.parser.add_argument('--exp_id', default='coco_dla')     #coco_dla was there
    ...
    self.parser.add_argument('--use_gaussian', action='store_true',
                              help = 'using gaussian weights to calculate the SSIM')
    self.parser.add_argument('--frame_gap', type=int, default=1,
                              help = 'Frame gap to be loaded. Default is current and previous.')
    self.parser.add_argument('--variant', type = int, default=1, 
                                    help = '1 - variant 1 without identity and with similarity map'
                                           '2 - variant 2 with identity and with mean dissimilarity map'
                                           '3 - variant 3 without identity and with dissimilarity map')
    self.parser.add_argument('--use_diff', action='store_true',
                              help = 'Just the scaled difference')
    self.parser.add_argument('--det_old', type=int, default=1,
                              help = '1 - Detection using old SSIM method')
    ...
    self.parser.add_argument('--arch', default='dla_34', 
                             help='model architecture. Currently tested'
                                  ...
                                  'stmdcn_50'
                                  'Insert other architectures here - format name_numlayers')
    ...
    self.parser.add_argument('--lr_step', type=str, default='25,40',
    ...
    self.parser.add_argument('--not_rand_crop', action='store_true',
                             help='not use the random crop data augmentation'
                                  'from CornerNet.')
    self.parser.add_argument('--shift', type=float, default=0.3,
                             help='when not using random crop'
                                  'apply shift augmentation.')
    self.parser.add_argument('--scale', type=float, default=0.4,
                             help='when not using random crop'
                                  'apply scale augmentation.')
    self.parser.add_argument('--rotate', type=float, default=0.25,
                             help='when not using random crop'
  ...
  def parse(self, args=''):
    ...
    print('Fixed size testing.' if opt.fix_res else 'Keep resolution testing.')
    ...
    opt.exp_dir = os.path.join(opt.root_dir,'..', '..', 'exp', opt.dataset, opt.arch, opt.input_type)
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    elif opt.task == 'ctdet' or opt.task == 'ctdet_smd' or opt.task == 'seaships' or opt.task == 'double' or opt.task == 'track' or opt.task == 'sm' or opt.task == 'doubleSM':
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [256, 256], 'num_classes': 80, 
                'mean': [0.40789654, 0.44719302, 0.47026115], 'std': [0.28863828, 0.27408164, 0.27809835],
                'dataset': 'coco'},
      'ucf24': {'default_resolution': [256, 256], 'num_classes': 24, 
                'mean': [0.40789654, 0.44719302, 0.47026115], 'std': [0.28863828, 0.27408164, 0.27809835],
                'dataset': 'ucf24'},
      'hmdb21': {'default_resolution': [256, 256], 'num_classes': 21, 
                'mean': [0.40789654, 0.44719302, 0.47026115], 'std': [0.28863828, 0.27408164, 0.27809835],
                'dataset': 'hmdb21'},
      'exdet': {'default_resolution': [512, 512], 'num_classes': 80, 
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'coco'},
      'smd': {'default_resolution': [512, 512], 'num_classes': 10, 
                'mean': [0.40789654, 0.44719302, 0.47026115], 'std': [0.28863828, 0.27408164, 0.27809835],
                'dataset': 'smd'},
      'seaships': {'default_resolution': [512, 512], 'num_classes': 6, 
                'mean': [0.40789654, 0.44719302, 0.47026115], 'std': [0.28863828, 0.27408164, 0.27809835],
                'dataset': 'seaships'}, 
      'flir': {'default_resolution': [512, 512], 'num_classes': 4, 
                'mean': [0.40789654, 0.44719302, 0.47026115], 'std': [0.28863828, 0.27408164, 0.27809835],
                'dataset': 'flir'},         
      'multi_pose': {
    ...
    opt = self.parse(args)
    dataset = Struct(default_dataset_info[opt.dataset])        # change to opt.dataset
