...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
    ...
    elif num_classes == 10 or dataset == 'smd':
      self.names = smd_class_name
    elif num_classes == 4 or dataset == 'flir':
      self.names = flir_class_name
    elif num_classes == 6 and dataset == 'seaships':
      ...
      self.W = 1242
      self.H = 375
    elif num_classes == 24 or dataset == 'ucf24':
      self.names = ucf24_class_name
      self.focal_length = 220
      self.W = 320
      self.H = 240
  ...
  def save_all_imgs(self, path='./cache/debug/', prefix='', genID=False):
    ...
    img_saved = sorted([int(x.split('_')[0]) for x in os.listdir(path) if '.png' in x])
    if len(img_saved) == 0:
      self.count = 0
    else:
      self.count = img_saved[-1]+1

    for i, v in self.imgs.items():
      cv2.imwrite(path + '/{:05d}_{}{}.png'.format(self.count, prefix, i), v)
...
ucf24_class_name = [  # always index 0
