...
def ctdet_post_process_hmap(hm, wh, reg, c, s, h, w, num_classes):
  batch, num_classes, height, width = hm.shape
  ys = np.arange(height, dtype = float)
  xs = np.arange(width, dtype = float)
  centers = np.zeros_like(wh, dtype=float)
  for b in range (batch):
    if reg is not None:
      centers[b, 0, :, :] = xs[:] + reg[b, 0, :, :]
      centers[b, 1, :, :] = ys[:, np.newaxis] + reg[b, 1, :, :]
    else:
      centers[b, 0, :, :] = xs[:] + 0.5
      centers[b, 1, :, :] = ys[:, np.newaxis] + 0.5
  
  tl_corners = centers - wh / 2
  br_corners = centers + wh / 2
  corners = np.concatenate((tl_corners, br_corners),axis=1)
  corners = corners.transpose(0, 2, 3, 1).reshape(batch, -1, 4)

  for i in range(batch):
    corners[i, :, 0:2] = transform_preds(
                          corners[i, :, 0:2], c[i], s[i], (w, h))
    corners[i, :, 2:4] = transform_preds(
                          corners[i, :, 2:4], c[i], s[i], (w, h))
  
  class_scores = hm.transpose(0,2,3,1).reshape(batch, -1, num_classes)

  return class_scores, corners
